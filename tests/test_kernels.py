...
class TestAttentionCore(unittest.TestCase):
    ...
    def test_attention_core_backward(self):
        ...
        q = torch.rand([n_seq, h, n_res, c], dtype=dtype, requires_grad=True).cuda()
        k = torch.rand([n_seq, h, n_res, c], dtype=dtype, requires_grad=True).cuda()
        v = torch.rand([n_seq, h, n_res, c], dtype=dtype, requires_grad=True).cuda()
        mask = torch.randint(0, 2, [n_seq, n_res]).cuda()
        mask_bias = (1e9 * mask - 1)[..., None, None, :].to(dtype)
        ...
        k_repro = clone(k)
        v_repro = clone(v)
        out_repro = attention_core(q_repro, k_repro, v_repro, mask_bias, None)
        ...
        k_gt = clone(k)
        v_gt = clone(v)
        out_gt = _attention(q_gt, k_gt, v_gt, [mask_bias])
        ...
        for t_repro, t_gt in pairs:
            self.assertTrue(torch.max(torch.abs(t_repro.grad - t_gt.grad)) < consts.eps)
